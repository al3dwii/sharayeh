...
module.exports = {
  ...
  /* --------------------------------------------------------- *
   * 2  Add dynamic /tools/ URLs even when they’re not static
   * --------------------------------------------------------- */
  additionalPaths: async () => {
    /** Dynamically import – avoids breaking the config if the file is TS */
    const { getConverters } = await import(
      './src/lib/server/converters.js' /* adjust if path differs */
    );
    const converters = getConverters();

    return converters.flatMap(({ slug_en, slug_ar }) => [
      {
        loc: `/en/tools/${slug_en}`,
        changefreq: 'weekly',
        priority: 0.8,
      },
      {
        loc: `/ar/tools/${slug_ar}`,
        changefreq: 'weekly',
        priority: 0.8,
      },
    ]);
  ...
  /* --------------------------------------------------------- *
   * 3  Per‑URL transform (kept from your original file)
   * --------------------------------------------------------- */
  transform: async (config, url) => {
    if (url.includes('markdown-to-powerpoint')) return null;

    const path = url.replace(config.siteUrl, ''); // e.g. '/en/tools/...'
