...
const CreatePresentation: React.FC = () => {
  ...
  // Updated state for user subscription tier
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      ...
      try {
        ...
        const response = await axios.get("/api/user-data", { // Updated endpoint
          headers: {
            Authorization: `Bearer ${token}`,
          },
        });
        setUserTier(response.data.tier.toLowerCase()); // Ensure tier is in lowercase
        console.log("📊 User Tier:", response.data.tier.toLowerCase());
      } catch (error) {
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files.length > 0) {
      ...
      // **Updated: Check if the user is uploading a file and is not premium**
      if (userTier !== "premium") {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      // Generate unique name without userId
      const uniqueName = generateUniqueName();

      // Create a new File record with unique name
      ...
      const response = await axios.post(
        "/api/files",
        {
          fileName,
          type: "PRESENTATION",
          uniqueName, // Include uniqueName here
        },
        {
      ...
      // Set Webhook URL to the centralized makeWebhook API
      const webhookUrl = "/api/makeWebhook";

      if (topicValue) {
        // For topic submissions, send JSON payload to makeWebhook
        const payload = {
          topic: topicValue,
          ...
          uniqueName: uniqueName, // Include uniqueName here
        };
        ...
        // For file submissions, send FormData to makeWebhook
        ...
        formData.append("uniqueName", uniqueName); // Include uniqueName here
