...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    try {
      // Assuming each user has one file record. Adjust as per your schema.
      await db.file.updateMany({
        where: { userId: userId }, // Update all files associated with the user
        data: { resultedFile2: processedFileUrl },
      });
      console.log('✅ Processed file URL saved to the database.');
    } catch (dbError: any) {
...
//   maxContentLength: Infinity, // Remove content length limit
//   maxBodyLength: Infinity,    // Remove body length limit
//   // responseType: 'arraybuffer', // Default response type for download
// });
...
//     // **Step 8: Generate a Unique Temporary File Path**
//     const uniqueId = `${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;
//     const tempDir = os.tmpdir();
//     uploadedFilePath = path.join(tempDir, `${uniqueId}${extension}`);
//     console.log(`📂 Temporary file path: ${uploadedFilePath}`);

//     // **Step 9: Save the Downloaded File to the Temporary Path**
//     console.log('💾 Saving file to temporary path');
//     await fs.promises.writeFile(uploadedFilePath, downloadResponse.data);
//     console.log('✅ File saved to temporary path.');

//     // **Step 10: Authenticate with Aspose API**
//     console.log('🔑 Authenticating with Aspose API');
//     const params = new URLSearchParams();
//     params.append('grant_type', 'client_credentials');
//     params.append('client_id', ASPOSE_CLIENT_ID);
...
//         params.toString()
//       );
//       console.log('🔑 Aspose Authentication Response:', {
//         access_token: authResponse.data.access_token ? 'Obtained' : 'Not Obtained',
...
//     // **Step 11: Define uploadPath (File Name Only)**
//     const uploadPath = originalFileName; // e.g., 'user_2q7QkRpIuwGAn2cU8bGg6CsPlOD.pptx'
//     console.log(`📤 Uploading file to Aspose Storage with uploadPath: ${uploadPath}`);
...
//     const API_VERSION = 'v3.0'; // Reverted to 'v3.0'

//     // **Step 12: Upload the File to Aspose Storage Sequentially**
//     console.log('📤 Uploading file to Aspose Storage.');
//     const uploadResponse = await axiosAsposeInstance.put(
...
//     // **Step 13: Verify Upload Response**
//     if (uploadResponse.status === 200 || uploadResponse.status === 201) {
//       console.log('✅ File uploaded to Aspose Storage.');
//     } else {
...
//           where: { id: fileId }, // Use the file ID from the request or the context
//           data: { resultedFile2: processedFileUrl },
//         });
//         console.log('✅ Processed file URL saved to the database.');
//       } catch (dbError) {
...
//     // **Step 21: Cleanup - Delete the Temporary Uploaded File**
//     if (uploadedFilePath && fs.existsSync(uploadedFilePath)) {
//       await fs.promises.unlink(uploadedFilePath);
//       console.log('🧹 Temporary file deleted:', uploadedFilePath);
//     }

//     // **Step 22: Return the Processed File URL to the Frontend**
//     console.log('📤 Returning response to frontend.');
//     return NextResponse.json(
//       {
//         success: true,
...
//     const API_VERSION = 'v4.0'; // Updated to 'v4.0'
